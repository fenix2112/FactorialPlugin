...
FMobileUtilsPlatform::FMobileUtilsPlatform()
{
	...
	{
		VibrateDeviceMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_VibrateDevice", "()Z", false);
...
bool FMobileUtilsPlatform::VibrateDevice()
{
	...
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		bResult = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, FMobileUtilsPlatform::VibrateDeviceMethod);
	}
	return bResult;
}
